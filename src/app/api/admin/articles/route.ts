...
// import { getArticles, getTotalArticlesCount } from '@/lib/placeholder-data'; // Not used
...
export async function GET(request: NextRequest) {
  console.log('[API Admin Get Articles] This route is disabled and will return 403.');
  return NextResponse.json({ success: false, message: 'Forbidden: Admin article management is currently disabled.' }, { status: 403 });
}

export async function POST(request: NextRequest) {
  console.log('[API Admin Create Article] This route is disabled and will return 403.');
  return NextResponse.json({ success: false, message: 'Forbidden: Admin article creation is currently disabled.' }, { status: 403 });
}
